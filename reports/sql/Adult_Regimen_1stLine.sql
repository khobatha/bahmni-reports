...
(Select Id, outcome, Age
	...
	(select distinct patient.patient_id AS Id,  'drug_count_1' as outcome,
				floor(datediff(CAST('2021-07-31' AS DATE), person.birthdate)/365) AS Age

                from obs o
						-- CLIENTS NEWLY INITIATED ON ART
						INNER JOIN patient ON o.person_id = patient.patient_id
                                 AND (o.concept_id = 3843 AND o.value_coded = 3841 OR o.value_coded = 3842)
								 AND MONTH(o.obs_datetime) = MONTH(CAST('2021-07-31' AS DATE)) 
								 AND YEAR(o.obs_datetime) = YEAR(CAST('2021-07-31' AS DATE))
								...
								CAST('2021-07-31' AS DATE) BETWEEN (DATE_ADD(DATE_ADD(person.birthdate, INTERVAL observed_age_group.min_years YEAR), INTERVAL observed_age_group.min_days DAY))
							...
							where o.person_id in (
									-- FOLLOW UPS
										...
										from obs oss
													...
													and CAST(oss.obs_datetime AS DATE) <= CAST('2021-07-31' AS DATE)
													and CAST(oss.obs_datetime AS DATE) >= DATE_ADD(CAST('2021-07-31' AS DATE), INTERVAL -13 MONTH)
													group by oss.person_id) firstquery
										inner join (
													select os.person_id,datediff(CAST(max(os.value_datetime) AS DATE), CAST('2021-07-31' AS DATE)) as last_ap
													from obs os
													where concept_id = 3752
													and CAST(os.obs_datetime AS DATE) <= CAST('2021-07-31' AS DATE)
									...
									(
										...
										group by os.person_id
										having latest_transferout <= CAST('2021-07-31' AS DATE)
									) as TOUTS
							)		
					...
					(
						...
						and o.person_id in (
								-- Death
											...
											from person p
											where dead = 1
											and death_date <= CAST('2021-07-31' AS DATE)		
						)
					)
 ...
 (Select Id, outcome, Age
	...
	(select distinct patient.patient_id AS Id,  'drug_count_1' as outcome,
				...
				INNER JOIN patient ON o.person_id = patient.patient_id
						...
						select active_clients.person_id
								...
								(select B.person_id, B.obs_group_id, B.value_datetime AS latest_follow_up
									...
									and obs_datetime <= cast('2021-07-31' as date)
									group by person_id) as A
									on A.observation_id = B.obs_group_id
									where concept_id = 3752
									and A.observation_id = B.obs_group_id	
								) as active_clients
								where active_clients.latest_follow_up >= cast('2021-07-31' as date)
		...
		and active_clients.person_id not in (
							...
							AND MONTH(os.obs_datetime) = MONTH(CAST('2021-07-31' AS DATE)) 
							AND YEAR(os.obs_datetime) = YEAR(CAST('2021-07-31' AS DATE))
							)
		...
		and active_clients.person_id not in (
							...
							AND MONTH(os.value_datetime) = MONTH(CAST('2021-07-31' AS DATE)) 
							AND YEAR(os.value_datetime) = YEAR(CAST('2021-07-31' AS DATE))
							)
		...
		and active_clients.person_id not in (
									select person_id 
									from person 
									where death_date <= cast('2021-07-31' as date)
									and dead = 1
						 )
						 )
						 -- end
	...
	inner join 
		...
		FROM(
					...
					AND o.voided = 0
					and o.obs_datetime <= cast('2021-07-31' as date)
 ...
 (select ID_, latest_follow_up,max_observation
		FROM(
			...
			and a.obs_datetime <= cast('2021-07-31' as date)
 ...
 where ARV_regimen in (2201,2203,2205,2207,3672,3673,4678,4679,4680,4681,4682,4683,2143))
...
(Select Id, outcome, Age
	FROM
	(select distinct patient.patient_id AS Id,  'drug_count_6' as outcome,
				floor(datediff(CAST('2021-07-31' AS DATE), person.birthdate)/365) AS Age
